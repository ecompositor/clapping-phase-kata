...
/**
  ...
  */
object Composer {
  def getPhase(original: Pattern, degrees: Int = 1): Pattern = {
    new Pattern(original.beats.drop(degrees) ++ original.beats.take(degrees))
  }

  def composeTwoPartPhaseMusic(original: Pattern): Stream[(Pattern, Pattern)] = {
